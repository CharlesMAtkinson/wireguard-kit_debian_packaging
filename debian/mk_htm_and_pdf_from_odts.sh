...
# Purpose: creates .htm and .pdf versions of .odt files

# Usage:
#   See function usage or run with -h option
...
function initialise {
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    [[ ! $opt_r_flag ]] && emsg+=$msg_lf'Option -r is required'
...
function mk_htm_and_pdf_from_odts {
    ...
    done < <(find "$root_dir" -type f -name '*.odt' -print0)
...
function mk_htm_or_pdf_from_odt {
    ...
    # ~~~~~~~~~~~~~~~~~~~~
    #   * soffice is difficult to error trap (0 exit status on error, error
    #     messages not documented) so any existing output file is removed
    #     before running soffice so output file creation can be used as a
    #     success indication
...
function usage {
    ...
    # Build the messages
    # ~~~~~~~~~~~~~~~~~~
    usage="usage: $my_name [-h] -r <root_dir>"
    msg='  where:'
    msg+=$'\n    -h prints this help and exits'
    msg+=$'\n    -r names the root of the tree to search for .odt files'
